...
PairInfo = recordtype('PairInfo', 'type direction cross closeA closeB dist')
...
for d, data in zip(['data'], [args.data]):
        ...
        for line in infile:
            ...
            for p in pairs:
                ...
                if (p[5], p[11]) not in relations[id_]:
                    relations[id_][(p[5], p[11])] = PairInfo(p[0], p[1], p[2], p[3], p[4], 0)
                else:
                    print('duplicates!')
    ...
    for id_ in relations.keys():
        for k, p in relations[id_].items():
            ...
            if p.cross == 'NON-CROSS' and (len(entities[id_][k[0]].sentNo.split(':')) == 1 and
                                       len(entities[id_][k[1]].sentNo.split(':')) == 1) and p.type == '1:CID:2':

                pp += 1
                # if len(entities[id_][k[0]].sentNo.split(':')) == 1:
                #     c = 0
                #     for e in entities[id_]:
                #         if e != k[0]:
                #             set1 = set(entities[id_][e].sentNo.split(':'))
                #             set2 = set(entities[id_][k[0]].sentNo.split(':'))
                #             cooc = list(set1 & set2)
                #             if len(cooc) == 1:
                #                 c += 1
                #     if c == 0:
                #         pp += 1
                #
                # elif len(entities[id_][k[1]].sentNo.split(':')) == 1:
                #     c = 0
                #     for e in entities[id_]:
                #         if e != k[1]:
                #             set1 = set(entities[id_][e].sentNo.split(':'))
                #             set2 = set(entities[id_][k[1]].sentNo.split(':'))
                #             cooc = list(set1 & set2)
                #             if len(cooc) == 1:
                #                 c += 1
                #     if c == 0:
    ...
    with open(args.output, 'w') as outfile:
        for id_ in relations.keys():
            for k, p in relations[id_].items():
                ...
                if p.type != '1:NR:2':
                    outfile.write('{}|{}|{}|{}|{}|{}\n'.format(id_, k[0], k[1], p.cross, p.dist, p.type))
...
print('inter-sentence pairs with 1 mention both 1 arg:', pp)
